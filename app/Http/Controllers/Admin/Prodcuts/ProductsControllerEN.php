...
use App\Models\ProductsEn;
use App\Helpers\FileUploadHelper;
use App\Http\Controllers\Controller;

class ProductsControllerEN extends Controller
{
    public function index() {
        $productsFromDB = ProductsEn::all();
        return view('backend.products.productsEn', ['products'=> $productsFromDB]);
    }
    ...
    public function store() {
        ...
        return to_route('admin.products-En')->with('success-create', 'تم اضافة العنصر بنجاح');
    }

    public function edit($id) {
        $productsFromDB = ProductsEn::where('id', $id)->first();
        return view('backend.products.product-editEn', ['product' => $productsFromDB]);
    }

    public function update(Request $request, $productID) {
        $product = ProductsEn::findOrFail($productID);
        request()->validate([
            'product_name' => ['required'],
            'product_description' => ['required'],
        ...
        return redirect()->route('admin.products-En')->with('success-update', 'تم تحديث المنتج بنجاح');
    }

    public function destroy($productID) {
        $product = ProductsEn::find($productID);
        $product -> delete();
        return to_route('admin.products-En')->with('success', 'تم حذف العنصر بنجاح');
    }
}
